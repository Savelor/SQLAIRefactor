...
This solution instructs the AI model (rule 18, see next paragraph) to detect and modify code that uses table variables or temporary tables with outdated data types. These data types are then automatically replaced with the recommended ones from Best Practices, ensuring greater consistency, maintainability, and alignment with modern standards.
